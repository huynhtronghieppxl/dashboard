...
function openBillDetail(r) {
    $(document).ready(function() {
        $('#table-detail-order').on('click', 'td #side-dish', function () {
            $(this).parent().find('.dropdown-side-dish').toggleClass('d-none')
        })
    });

    thisBonusPunish = r;
    idBillDetailBillManage = r.data('id');
    $('#modal-detail-bill-manage').modal('show');
...
async function dataBillDetail(id) {
    ...
    $('#out-detail-bill-manage').text(res.data[4].payment_date);
    $('#date-detail-bill-manage').text(res.data[4].date);
    $('#money-detail-bill-manage').text(formatNumber(res.data[4].config.data.amount));
    ...
    // $('#accumulate-point-detail-bill-manage').html(`${res.data[4].membership_promotion_point_used_amount} ( ${res.data[4].membership_promotion_point_used} Điểm)`);
    $('#recharge-point-detail-bill-manage').html(`${res.data[4].membership_point_used_amount} ( ${res.data[4].membership_point_used} Điểm)`);
    $('#total-point-payment-detail-bill-manage').text(res.data[4].total_final_amount);
    switch (res.data[4].discount_type) {
        case 1:
            $('#detail-discount-bill-manage').text('Tổng bill');
            break;
        case 2:
            $('#detail-discount-bill-manage').text('Món ăn');
            break;
        case 3:
            $('#detail-discount-bill-manage').text('Thức uống');
            break;
    }
...
async function tableDetailBillManage(data) {
    let id1 = $('#table-detail-order'),
        ...
        column1 = [
            ...
            {data: 'category_food', name: 'category_food', className: 'text-left'},
...
function closeModalDetailBillManage() {
    $('#modal-detail-bill-manage').modal('hide');
