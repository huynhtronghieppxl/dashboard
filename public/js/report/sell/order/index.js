...
$(async function () {
    ...
    $(document).on('click', '#search-btn-bill-liabilities', function () {
        ...
        if (moment(fromListBill, 'DD/MM/YYYY').clone().format('x') > moment(to, 'DD/MM/YYYY').clone().format('x')) {
            ...
            tableWaitingSupplier.ajax.url("order-report.data?from=" + fromListBill + "&to=" + toListBill + "&branch_id=" + branchIdListBill + "&restaurant_brand_id=" + restaurantBrandId + "&filter_status_order=" + filterStatusOrder).load();
        }
    });
    $('#list-bill-select-status').on('select2:select', function () {
        statusListBill = $(this).val();
        data_table.ajax.url("order-report.data?from=" + fromListBill + "&to=" + toListBill + "&branch_id=" + branch_id + "&status=" + statusListBill + "&restaurant_brand_id=" + restaurantBrandId + "&filter_status_order=" + filterStatusOrder).load();
    });
    await loadDataBill();
});
...
async function loadDataBill() {
    let element = $('#table-order'),
        url = "order-report.data?from=" + fromListBill + "&to=" + toListBill + "&branch_id=" + branchIdListBill + "&restaurant_brand_id=" + restaurantBrandId + "&filter_status_order=" + filterStatusOrder,
        fixedLeftTable = 0,
        fixedRightTable = 0,
        column = [
