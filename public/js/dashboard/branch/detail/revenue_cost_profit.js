...
async function dataTableDetailRevenueCostProfit(typeApi, key) {
    ...
    if (typeApi === 0) {
        ...
        url = "branch-dashboard.data-detail-revenue-report?brand=" + brand + "&branch=" + branch + "&type=" + type + "&time=" + time + "&object_type=" + key;
    } else {
        $('#title-detail-revenue-cost-profit-branch-report').text('Chi tiết chi phí');
        url = "branch-dashboard.data-detail-cost-report?brand=" + brand + "&branch=" + branch + "&type=" + type + "&time=" + time + "&is_current=" + key;
    }
