...
function dataProfitLossReport() {
    ...
    axios({
        ...
        $('#sell-revenue-P-L').text(res.data[0])
        $('#sell-total-cost-P-L').text(res.data[2])
        $('#sell-total-profit-P-L').text(res.data[4])
...
function chartPieProfitLossEchartTemplate(data, element) {
    if (data.length === 0) {
        $('#' + element).html(
            "<div class='empty-datatable-custom center-loading'><img src='../../../images/tms/empty.png' style='width: 200px'></div>"
        );
        return false;
    }
