...
class InInventoryController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($config['data']['list']); $i++) {
                ...
                $config['data']['list'][$i]['paid_status_name'] = $this->paidStatusDataInInventoryManage($config['data']['list'][$i]['paid_status']);
                ...
                $config['data']['list'][$i]['action'] = '<div class="btn-group btn-group-sm">
                                                            <button type="button" class="tabledit-edit-button btn btn-primary waves-effect waves-light" onclick="openDetailInInventoryManage($(this))" data-id="' . $config['data']['list'][$i]['id'] . '" data-branch="' . $config['data']['list'][$i]['branch']['id'] . '" data-toggle="tooltip" data-placement="top" data-original-title="' . TEXT_DETAIL . '"><i class="fi-rr-eye"></i></button>
                                                        </div>';
    ...
    public function paidStatusDataInInventoryManage($status)
    {
        switch ($status) {
