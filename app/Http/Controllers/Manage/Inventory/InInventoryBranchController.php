...
class InInventoryBranchController extends Controller
{
    public function index(Request $request)
    {
        ...
        $active_nav = 'Nhập từ chi nhánh khác';
        return view('manage.inventory.in_inventory_branch.index', compact('active_nav'));
    }
        ...
        try {
            for ($i = 0; $i < count($configAll[0]['data']['list']); $i++) {
                ...
                $configAll[0]['data']['list'][$i]['action'] = '<div class="btn-group btn-group-sm ">
                                                                <button type="button" class="tabledit-edit-button btn seemt-btn-hover-blue waves-effect waves-light" onclick="openDetailInInventoryBranchManage(' . $configAll[0]['data']['list'][$i]['id'] . ',' . $configAll[0]['data']['list'][$i]['branch']['id'] . ')" data-toggle="tooltip" data-placement="top" data-original-title="' . TEXT_DETAIL . '"><i class="fi-rr-eye"></i></button>
                                                            </div>';
            }
            ...
            $data_table = Datatables::of($config['data']['details'])
                ...
                ->addColumn('action', function ($row) {
                    return '<div class="btn-group btn-group-sm text-center">
                                <button type="button" class="tabledit-edit-button btn seemt-btn-hover-blue waves-effect waves-light" onclick="openModalDetailMaterialData(' . $row['restaurant_material_id'] . ')" title="Chi tiết"><i class="fi-rr-eye"></i></button>
                            </div>';
                })
