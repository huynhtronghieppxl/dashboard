...
class CancelInventoryController extends Controller
{
    public function index ()
    {
        ...
        $active_nav = 'Hủy hàng kho tổng';
        return view('manage.warehouse.cancel_inventory.index', compact('active_nav'));
    }
        ...
        try {
            ...
            for ($i = 0; $i < count($config['data']['list']); $i++) {
                ...
                $config['data']['list'][$i]['action'] = '<div class="btn-group btn-group-sm">
                                                            <button type="button" class="tabledit-edit-button btn seemt-btn-hover-blue waves-effect waves-light" onclick="openDetailCancelInventoryWarehouse($(this))" data-id="' . $config['data']['list'][$i]['id'] . '" data-branch="' . $config['data']['list'][$i]['branch']['id'] . '" data-toggle="tooltip" data-placement="top" data-original-title="' . TEXT_DETAIL . '"><i class="fi-rr-eye"></i></button>
                                                         </div>';
            }
            $data_table = array(
    ...
    public function create(Request $request)
    {
        ...
        $api = API_CANCEL_INVENTORY_MANAGE_GET_LIST;
        $body = [
