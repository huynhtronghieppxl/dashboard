...
class PromotionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $active_nav = 'customer.promotion';
        return view('customer.promotion.index', compact('active_nav'));
    }
...
//        dd($config);
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row['status'] === (int)Config::get('constants.type.RestaurantPromotionTypeStatusEnum.EXPIRED')) {
                    $action = '<div class="btn-group btn-group-sm ml-auto">
                                   <button type="button" class="tabledit-edit-button btn btn-primary waves-effect waves-light" onclick="openModalDetailPromotion(' . $row['id'] . ')" title="' . TEXT_DETAIL . '"><span class="icofont icofont-eye-alt"></span></button>
                               </div>';
                } else if ($row['status'] === (int)Config::get('constants.type.RestaurantPromotionTypeStatusEnum.APPLYING')) {
                    $action = '<div class="btn-group btn-group-sm ml-auto">
                                   <button type="button" class="tabledit-edit-button btn btn-inverse waves-effect waves-light" onclick="pausePromotion(' . $row['id'] . ')" title="' . $pause . '"><span class="fa fa-bell-slash"></span></button>
                                   <button type="button" class="tabledit-edit-button btn btn-primary waves-effect waves-light" onclick="openModalDetailPromotion(' . $row['id'] . ')" title="' . TEXT_DETAIL . '"><span class="icofont icofont-eye-alt"></span></button>
                               </div>';
                } else if ($row['status'] === (int)Config::get('constants.type.RestaurantPromotionTypeStatusEnum.PAUSING')) {
                    $action = '<div class="btn-group btn-group-sm ml-auto">
                                   <button type="button" class="tabledit-edit-button btn btn-success waves-effect waves-light" onclick="applyPromotion(' . $row['id'] . ')" title="' . TEXT_APPLY . '"><span class="fa fa-bell-o"></span></button>
                                   <button type="button" class="tabledit-edit-button btn btn-warning waves-effect waves-light" onclick="openModalUpdateCustomerPromotion(' . $row['id'] . ')" title="' . TEXT_UPDATE . '"><span class="icofont icofont-ui-edit"></span></button>
                                   <button type="button" class="tabledit-edit-button btn btn-primary waves-effect waves-light" onclick="openModalDetailPromotion(' . $row['id'] . ')" title="' . TEXT_DETAIL . '"><span class="icofont icofont-eye-alt"></span></button>
                               </div>';
    ...
    public function create(Request $request)
    {
        ...
        $api = sprintf(API_PROMOTION_POST_CREATE);
        $body = [
...
//        $project_id = Config::get('constants.GATEWAY.PROJECT_ID.ORDER');
//        $method = Config::get('constants.GATEWAY.METHOD.POST');
//        $api = sprintf(Config::get('constants.api.API_POST_APPLY_PROMOTION'), $id));
//
//
//        $body = null;
//        return $this->callApiGatewayTemplate2($project_id, $method, $api, $body);
...
//        $project_id = Config::get('constants.GATEWAY.PROJECT_ID.ORDER');
//        $method = Config::get('constants.GATEWAY.METHOD.POST');
//        $api = sprintf(Config::get('constants.api.API_POST_PAUSE_PROMOTION'), $id));
//
//
//        $body = null;
//        return $this->callApiGatewayTemplate2($project_id, $method, $api, $body);
//    }
    ...
    public function dataFood(Request $request)
    {
        ...
        $method = Config::get('constants.GATEWAY.METHOD.GET');
        $api = sprintf(API_FOOD_GET_ALL_MANAGE, $status, $is_take_away, $is_addition, $category_type, $category_id, $restaurant_brands_id, $branch, $is_count_material, $page, $limit, $is_bestseller, $is_combo, $is_kitchen, $is_special_gift, $key_search, $is_get_food_contain_addition, $alert_original_food_id);
        $body = null;
    ...
    public function promotionAssign(Request $request)
    {
        ...
        try {
            $data = collect($config['data']['list']);
            $data = array_values($data->filter(function ($val) {
                return strstr($val['type_status'], Config::get('constants.type.RestaurantPromotionTypeStatusEnum.APPLYING')) ||
                    strstr($val['type_status'], Config::get('constants.type.RestaurantPromotionTypeStatusEnum.PENDDING')) ||
                    strstr($val['type_status'], Config::get('constants.type.RestaurantPromotionTypeStatusEnum.PAUSING'));
            })->toArray());
    ...
    public function promotionAssignDetail(Request $request)
    {
        ...
        try {
            ...
            if ($data['type_status'] === (int)Config::get('constants.type.RestaurantPromotionTypeStatusEnum.APPLYING') || $data['type_status'] === (int)Config::get('constants.type.RestaurantPromotionTypeStatusEnum.PAUSING')) {
                $button = '';
            } else {
    ...
    public function assignFood(Request $request)
    {
        $project_id = Config::get('constants.GATEWAY.PROJECT_ID.ORDER');
        $method = Config::get('constants.GATEWAY.METHOD.POST');
        $api = sprintf(API_PROMOTION_POST_ASSIGN_FOOD);
        $body = [
            'restaurant_brand_id' => $request->get('restaurant_brand_id'),
