...
class CostReportController extends Controller
{
    public function index()
    {
        ...
        $active_nav = 'Báo cáo chi phí';
        return view('report.cost.index', compact('active_nav'));
    }
    ...
    public function data(Request $request)
    {
        ...
        try {
            ...
            $data_table = DataTables::of($config['data'])
                ...
                ->addColumn('action', function ($row) use ($type, $time) {
                    if ($row['is_automatically_generated'] === 1) {
                        return '<div class="btn-group btn-group-sm">
                            <button type="button" class="tabledit-edit-button btn seemt-btn-hover-blue waves-effect waves-light" data-original-title="' . TEXT_DETAIL . '" data-id="' . $row['addition_fee_reason_id'] . '" data-type-action="'. $type .'" data-time-action="'. $time .'" data-type-id="' . $row['addition_fee_reason_type_id'] . '" data-name="' . $row['addition_fee_reason_content'] . '" data-type="' . TEXT_ADDITION_FEE_AUTO_PAYMENT . '" data-amount="' . $this->numberFormat($row['amount']) . '" data-auto="' . $row['is_automatically_generated'] . '" data-generated-type="2" onclick="openModalDetailCostReport($(this))"><i class="fi-rr-eye"></i></button>
                         </div>';
                    } else {
                        return '<div class="btn-group btn-group-sm">
                            <button type="button" class="tabledit-edit-button btn seemt-btn-hover-blue waves-effect waves-light" data-original-title="' . TEXT_DETAIL . '" data-id="' . $row['addition_fee_reason_id'] . '" data-type-action="'. $type .'" data-time-action="'. $time .'" data-type-id="' . $row['addition_fee_reason_type_id'] . '" data-name="' . $row['addition_fee_reason_content'] . '" data-type="' . TEXT_ADDITION_FEE_PAYMENT . '" data-amount="' . $this->numberFormat($row['amount']) . '" data-auto="' . $row['is_automatically_generated'] . '" data-generated-type="-1" onclick="openModalDetailCostReport($(this))"><i class="fi-rr-eye"></i></button>
                         </div>';
                    }
                })
                ...
                ->make(true);
            $data_total = [
                'total_amount_cost' => $this->numberFormat(array_sum(array_column($config['data'], 'amount'))),
            ...
            return [$data_chart_pie, $data_chart, $data_table, $data_total, $config];
        } catch (Exception $e) {
    ...
    function detail(Request $request)
    {
        ...
        if ((int)$request->get('auto') === ENUM_SELECTED) {
            ...
            $addition_fee_reason_id = $request->get('id');
            $addition_fee_reason_type_id = ENUM_DIS_SELECTED;
        }
