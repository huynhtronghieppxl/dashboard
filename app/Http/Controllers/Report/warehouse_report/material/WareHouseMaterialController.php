...
namespace App\Http\Controllers\report\warehouse_report\material;
...
class WareHouseMaterialController extends Controller
{
    public function index(Request $request)
    {
        ...
        $active_nav = 'Nhập kho';
        return view('report.warehouse_report.material.index', compact('active_nav'));
    }
    ...
    public function drawTableMaterialReport($data)
    {
        return Datatables::of($data)
            ...
            ->addColumn('receive_from_supplier_amount', function ($row) {
                ...
                return '<label class="' . $class . '">' . $this->numberFormat($row['total_receive_from_supplier_amount'] - $row['total_return_amount']) . '</label><br>
                       <label class="number-order ' . $class . '"><em style="color: #9d9d9de6">SL: </em>' . $this->numberFormat($row['receive_from_supplier_quantity'] - $row['return_quantity']) . '</label>';
            })
